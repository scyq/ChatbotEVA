    ...
    def add(self, hyp, sum_logprobs):
        ...
        if len(self) < self.num_beams or score > self.worst_score:
            ...
            if len(self) > self.num_beams:
                ...
                self.raw_worst_score = self.worst_score * (len(
                    sorted_scores[1][2])**self.length_penalty)
            else:
...
def construct_antonym_dict(args):
    if args.rule_path is None:
        return None
    with open(os.path.join(args.rule_path, './antonym/antonym.txt'),
              'r',
              encoding="utf-8") as f:
        data = f.read().split("\n")
...
def calc_banned_ngram_tokens(prev_input_ids, num_hypos: int,
                             no_repeat_ngram_size: int,
                             tokenizer: EVATokenizer) -> None:
        ...
        for ngram in zip(*[gen_words[i:]
            ...
            for prefix_len in range(no_repeat_ngram_size):
                ...
                if tokenizer.check(suffix_ngram):  # 在词表中
                    generated_ngram[prev_ngram] = generated_ngram.get(
                        prev_ngram, set()) | set([suffix_ngram])
    ...
    def _get_generated_ngrams(hypo_idx):
        ...
        '''
        for prefix_len in range(no_repeat_ngram_size):
            ngram_words = ''.join(prev_input_words[hypo_idx][cur_len -
                                                             prefix_len:])
            generated_ngram_words = generated_ngrams[hypo_idx].get(
                ngram_words, [])
...
def enforce_repetition_penalty_(tokenizer, lprobs, batch_size, num_beams,
...
def postprocess_next_token_scores(
    ...
    banned_tokens = calc_banned_antonym_words_ids(input_ids, tokenizer,
                                                  antonym_dict)
...
def generate_no_beam(model_batch, full_context, model, tokenizer: EVATokenizer,
                     args, device):
    ...
    enc_outputs = model(enc_input_ids=enc_input_ids,
    ...
    cross_attention_mask = model_batch['cross_attention_mask'][
        ..., :dec_init_length, :]

    unfinished_sents = enc_input_ids.new(enc_input_ids.size(0)).fill_(1)
    output_ids = enc_input_ids.new_zeros([enc_input_ids.size(0),
                                          0])  # not include the prompt
    ...
    while gen_len < target_length:
        if unfinished_sents.max() == 0:
            ...
            gathered_lm_logits = [
                torch.zeros_like(lm_logits).to(device)
                for _ in range(mpu.get_model_parallel_world_size())
            ]
            torch.distributed.all_gather(gathered_lm_logits, lm_logits.data,
            ...
            tokens_to_add = next_token * unfinished_sents + tokenizer.sep_id * (
                1 - unfinished_sents)
            ...
            dec_attention_mask = torch.cat([
                dec_attention_mask[:, :, -1:, :], dec_attention_mask[:, :, -1:,
                                                                     -1:]
            ],
                                           dim=-1)
    ...
    generation_str_list = []
    for e in output_ids:
        generation_token_ids = e[:e.index(tokenizer.sep_id
                                          )] if tokenizer.sep_id in e else e
...
def generate_beam(model_batch, full_context, model, tokenizer: EVATokenizer,
                  args, device):
    '''
    ...
    batch_size = enc_input_ids.size(0)
    enc_input_ids = enc_input_ids.unsqueeze(1).expand(batch_size, num_beams,
                                                      enc_input_length)
    enc_attention_mask = enc_attention_mask.unsqueeze(1).expand(
        batch_size, num_beams, 1, enc_input_length, enc_input_length)

    enc_input_ids = enc_input_ids.contiguous().view(batch_size * num_beams,
                                                    enc_input_length)
    enc_attention_mask = enc_attention_mask.contiguous().view(
        batch_size * num_beams, 1, enc_input_length, enc_input_length)

    full_context = full_context.unsqueeze(1).expand(batch_size, num_beams,
                                                    full_context.size(-1))
    full_context = full_context.contiguous().view(batch_size * num_beams,
                                                  full_context.size(-1))
    ...
    dec_input_ids = dec_input_ids.unsqueeze(1).expand(batch_size, num_beams,
                                                      dec_init_length)
    ...
    dec_input_ids = dec_input_ids.contiguous().view(batch_size * num_beams,
                                                    dec_init_length)
    ...
    while gen_len < target_length:
        ...
        beam_scores = torch.tensor([x[0] for x in next_batch_beam],
                                   device=dec_input_ids.device)
